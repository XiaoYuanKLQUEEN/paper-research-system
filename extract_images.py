...
def extract_arxiv_source(arxiv_id, temp_dir):
    """下载并提取arXiv源码包（尝试多个镜像）"""
    mirrors = [
        f"https://arxiv.org/e-print/{arxiv_id}",
        f"https://xxx.itp.ac.cn/e-print/{arxiv_id}",
        f"https://cn.arxiv.org/e-print/{arxiv_id}",
    ]
        ...
        try:
            if HAS_REQUESTS:
                ...
                try:
                    ...
                    logger.warning("HTTP错误 %d: %s, 尝试下一个镜像", http_err.code, http_err.reason)
                    continue
            ...
            if status == 200 and content:
                ...
                print(f"下载失败: HTTP {status}, 尝试下一个镜像")
        except Exception as e:
            logger.warning("下载源码包失败 (%s): %s, 尝试下一个镜像", source_url[:50], e)
    
    logger.error("所有镜像均下载失败")
    return False
