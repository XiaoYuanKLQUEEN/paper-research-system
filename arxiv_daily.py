...
# arXiv API 地址（优先中科院镜像，回退官方）
ARXIV_API_MIRRORS = [
    "https://export.arxiv.org/api/query",     # 官方
    "https://xxx.itp.ac.cn/api/query",        # 中科院镜像
    "https://cn.arxiv.org/api/query",         # 国内镜像
]
...
def search_arxiv_by_date_range(
    ...
    for mirror_api in ARXIV_API_MIRRORS:
        mirror_url = (
            f"{mirror_api}?"
        ...
        logger.debug("[arXiv] Trying mirror: %s...", mirror_url[:80])
        
        for attempt in range(max_retries):
            try:
                with urllib.request.urlopen(mirror_url, timeout=60) as response:
                    xml_content = response.read().decode('utf-8')
                    papers = parse_arxiv_xml(xml_content)
                    logger.info("[arXiv] Found %d papers (mirror: %s)", len(papers), mirror_api[:40])
                    return papers
            except Exception as e:
                logger.warning("[arXiv] Error on %s (attempt %d/%d): %s", mirror_api[:40], attempt + 1, max_retries, e)
                if attempt < max_retries - 1:
                    wait_time = (2 ** attempt) * 2
                    time.sleep(wait_time)
                else:
                    break  # 尝试下一个镜像
        logger.warning("[arXiv] Mirror %s exhausted, trying next...", mirror_api[:40])
    
    logger.error("[arXiv] All mirrors failed after %d attempts each", max_retries)
    return []
    ...
    for mirror_api in ARXIV_API_MIRRORS:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning("[arXiv] Keyword search error on %s (attempt %d/%d): %s", mirror_api[:40], attempt + 1, max_retries, e)
                if attempt < max_retries - 1:
                    wait_time = (2 ** attempt) * 2
                    time.sleep(wait_time)
                else:
                    break
        logger.warning("[arXiv] Mirror %s exhausted for keyword search, trying next...", mirror_api[:40])
    
    logger.error("[arXiv] Keyword search failed after all mirrors")
    return []
    ...
    try:
        ...
        for entry in root.findall('atom:entry', ARXIV_NS):
            ...
            if id_elem is not None:
                ...
                # URL 格式：http://arxiv.org/abs/2606.02578v1
                match = re.search(r'/abs/(\d{4}\.\d{4,5})', paper['id'])
                if match:
